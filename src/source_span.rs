...
pub(crate) struct TestSourceSpan {
    filename: String,
    line: usize,
    col: usize,
    #[source_code]
...
impl TestSourceSpan {
    ...
    pub(crate) fn add_related<E: Into<Report>>(&mut self, err: E) {
        self.related.push(err.into());
    }

    /// Tries to find source information from backtrace.
    pub(crate) fn from_backtrace(
        build_manifest_dir: impl AsRef<Path>,
    ) -> Result<Option<Self>, Box<dyn std::error::Error>> {
        let build_manifest_dir = build_manifest_dir.as_ref();
